...
def chunk_uuid( cfg: AppConfig, document: Document, chunk_id: int ) -> str:
    """
    ...
    """
    src = str(document.metadata.get("source_file", "unknown"))
    page = str(document.metadata.get("page_human", ""))
    content_hash = hashlib.sha1((document.page_content or "").encode("utf-8")).hexdigest()[:16]
...
def ingest_incremental(
    ...
    with timed(f"ingest:{table_name}"):
        raw_documents_from_disk: list[Document] = load_documents_from_dir(docs_dir)
        split_chunks: list[Document] = split_documents(raw_documents_from_disk, config.chunk_size, config.chunk_overlap)

        ids: list[str] = []
        for i, document in enumerate(split_chunks):
        ...
        existing = fetch_existing_ids(pg_dsn, config.schema, table_name, ids)
        missing_docs = []
        missing_ids = []
        for document, id_ in zip(split_chunks, ids):
